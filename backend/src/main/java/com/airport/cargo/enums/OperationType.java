...
public enum OperationType {
    ...
    CUSTOMS_DRAW("CUSTOMS_DRAW", "海关抽中"),
    CUSTOMS_INSPECT("CUSTOMS_INSPECT", "开始查验"),
    CUSTOMS_PASS("CUSTOMS_PASS", "查验通过放行"),
    CUSTOMS_HOLD("CUSTOMS_HOLD", "查验暂扣"),
    PARTIAL_RELEASE("PARTIAL_RELEASE", "部分放行(部分抽检)"),
    JOIN_QUEUE("JOIN_QUEUE", "加入排队"),
    LEAVE_QUEUE("LEAVE_QUEUE", "离开排队"),
    REQUEUE("REQUEUE", "重新排队"),
    QUEUE_RECALCULATE("QUEUE_RECALCULATE", "队列重算"),
    WINDOW_RECALCULATE("WINDOW_RECALCULATE", "到场窗口重算"),
    SECURITY_CHECK("SECURITY_CHECK", "安保检查"),
    SECURITY_REJECT("SECURITY_REJECT", "安保驳回"),
    VOUCHER_REISSUE("VOUCHER_REISSUE", "放行凭证重开"),
